...
gameTieMessage: .asciiz "The game has resulted in a tie!"
...
correctComputerMove:
	li $t1, 2
	sw $t1, ($s0) #store computer numerical identity into that address location
	jal checkWins
...
rOneCh:	
	lw $t0, 0($s0)
	beq $t0, $zero, rTwoCh #if we see an empty, this row has no winner
	lw $t1, 4($s0)
	beq $t1, $zero, rTwoCh #if we see an empty, this row has no winner
	lw $t2, 8($s0)
	beq $t2, $zero, rTwoCh #if we see an empty, this row has no winner
	seq $t0, $t0, $t1 #set t0 equal to both t0 and t1
	seq $t1, $t1, $t2 #set t1 equal to both t1 and t2
	and $t0, $t0, $t1 #if both t0 and t1 are same, we have a winner.
	bne $t0, $zero, rWin # move to win, $t2 will have the number of which won
	j rTwoCh	
rTwoCh:	
	lw $t0, 12($s0)
	beq $t0, $zero, rThrCh #if we see an empty, this row has no winner
	lw $t1, 16($s0)
	beq $t1, $zero, rThrCh #if we see an empty, this row has no winner
	lw $t2, 20($s0)
	beq $t2, $zero, rThrCh #if we see an empty, this row has no winner
	seq $t0, $t0, $t1
	seq $t1, $t1, $t2
	and $t0, $t0, $t1
	bne $t0, $zero, rWin # move to win, $t2 will have the number of which won
...
rThrCh:	
	lw $t0, 24($s0)
	beq $t0, $zero, rReturn #if we see an empty, this row has no winner
	lw $t1, 28($s0)
	beq $t1, $zero, rReturn #if we see an empty, this row has no winner
	lw $t2, 32($s0)
	beq $t2, $zero, rReturn #if we see an empty, this row has no winner
	seq $t0, $t0, $t1 #set equal t0 to both t0 and t1
	seq $t1, $t1, $t2 #set equal t1 to both t1 and t2
	and $t0, $t0, $t1 #if both are equal, t0 will be 1. this means we have a winner
	bne $t0, $zero, rWin # move to win, $t2 will have the number of which won
	j rReturn
rWin:	
	j finishGame
...
cOneCh:	
	lw $t0, 0($s0)
	beq $t0, $zero, cTwoCh #if we see an empty, this col has no winner
	lw $t1, 12($s0)
	beq $t1, $zero, cTwoCh #if we see an empty, this col has no winner
	lw $t2, 24($s0)
	beq $t2, $zero, cTwoCh #if we see an empty, this col has no winner
	seq $t0, $t0, $t1
	seq $t1, $t1, $t2
	and $t0, $t0, $t1
	bne $t0, $zero, rWin # move to win, $t2 will have the number of which won
...
cTwoCh:	
	lw $t0, 4($s0)
	beq $t0, $zero, cThrCh #if we see an empty, this col has no winner
	lw $t1, 16($s0)
	beq $t1, $zero, cThrCh #if we see an empty, this col has no winner
	lw $t2, 28($s0)
	beq $t2, $zero, cThrCh #if we see an empty, this col has no winner
	seq $t0, $t0, $t1
	seq $t1, $t1, $t2
	and $t0, $t0, $t1
	bne $t0, $zero, rWin # move to win, $t2 will have the number of which won
...
cThrCh:	
	lw $t0, 8($s0)
	beq $t0, $zero, cReturn #if we see an empty, this col has no winner
	lw $t1, 20($s0)
	beq $t1, $zero, cReturn #if we see an empty, this col has no winner
	lw $t2, 32($s0)
	beq $t2, $zero, cReturn #if we see an empty, this col has no winner
	seq $t0, $t0, $t1
	seq $t1, $t1, $t2
	and $t0, $t0, $t1
...
dOneCh:	
	lw $t0, 0($s0)
	beq $t0, $zero, dTwoCh #if we see an empty, this diag has no winner
	lw $t1, 16($s0)
	beq $t1, $zero, dTwoCh #if we see an empty, this diag has no winner
	lw $t2, 32($s0)
	beq $t2, $zero, dTwoCh #if we see an empty, this diag has no winner
	seq $t0, $t0, $t1
	seq $t1, $t1, $t2
	and $t0, $t0, $t1
...
dTwoCh:	
	lw $t0, 8($s0)
	beq $t0, $zero, dReturn #if we see an empty, this diag has no winner
	lw $t1, 16($s0)
	beq $t1, $zero, dReturn #if we see an empty, this diag has no winner
	lw $t2, 24($s0)
	beq $t2, $zero, dReturn #if we see an empty, this diag has no winner
	seq $t0, $t0, $t1
	seq $t1, $t1, $t2
	and $t0, $t0, $t1
	bne $t0, $zero, dWin # move to win, $t2 will have the number of which won
...
finishGame:
	...
	beq $t2, 1, playerWon #if t2, the current player, is 1, then player won
	beq $t2, 2, computerWon #if t2, the current player, is 2, then computer won
playerWon:
	li $v0 4,
	la $a0, playerWonStr
	syscall
